...
- **The service-worker `SHELL` (`sw.js`) must list every module statically
  imported from `renderer.js` / `index.html`.** It's hand-maintained with no
  guard; a missing entry = blank app on an offline reload after an update (this
  shipped in rc.59 for `pwa-install.js`, fixed in rc.60). Add new modules here.
...
- **Find highlights + line numbers depend on THREE elements sharing identical
  ...
  wrap on one → change all three, or marks and gutter numbers silently drift. No
  test guards this yet.
